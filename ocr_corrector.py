...
class OCRCorrector:
    ...
    def _get_checkpoint(self):
        """See DiagramAnalyzer._get_checkpoint for the full explanation --
        this wires the same, previously-unused checkpoint.py API cache into
        OCR text correction."""
        ...
        try:
            ...
            logger.debug(f"  Checkpoint DB unavailable for OCR correction cache: {e}")
            self._checkpoint = False
        return self._checkpoint or None
    ...
    def correct(self, ocr_result: OCRResult, context_hint: str = "") -> CorrectedText:
        ...
        # 5. AI correction (Claude, with Gemini as a free-tier fallback —
        #    handles domain terms, equations, etc.)
        if self.cfg.enable_ai_correction and (self.api_cfg.anthropic_api_key or self.api_cfg.gemini_api_key):
            ai_corrected = self._ai_correct(text, context_hint)
    ...
    def _ai_correct(self, text: str, context_hint: str = "") -> str:
        """
        ...
        Cached by (text + context_hint) via the checkpoint DB, same
        rationale as DiagramAnalyzer._ai_analyze -- re-running the same PDF
        during iteration shouldn't re-pay for/re-call correction on
        unchanged OCR text.
        """
        ckpt = self._get_checkpoint()
        cache_model_key = "ocr_correction_v1"
        cache_key_text = f"{context_hint}\x00{text}".encode("utf-8", "ignore")
        if ckpt:
            cached = ckpt.get_cached_api_response(cache_key_text, "ocr_correct", cache_model_key)
            if cached is not None:
                logger.debug("  OCR correction: cache hit")
                return cached
        ...
        for provider in providers:
            if provider == "anthropic" and self.api_cfg.anthropic_api_key:
                try:
                    result = self._claude_correct(text, context_hint)
                    if ckpt:
                        ckpt.cache_api_response(cache_key_text, "ocr_correct", cache_model_key, result)
                    return result
                except Exception as e:
                    logger.warning(f"  Claude OCR correction failed, trying next provider: {e}")
            elif provider == "gemini" and self.api_cfg.gemini_api_key and GEMINI_AVAILABLE:
                try:
                    result = self._gemini_correct(text, context_hint)
                    if ckpt:
                        ckpt.cache_api_response(cache_key_text, "ocr_correct", cache_model_key, result)
                    return result
                except Exception as e:
                    logger.warning(f"  Gemini OCR correction failed, trying next provider: {e}")
        return text
...
--- END ---"""
    ...
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(min=2, max=20))
    def _gemini_correct(self, text: str, context_hint: str = "") -> str:
        """Use Gemini (free tier) to fix OCR errors when no Claude key is set."""
...
--- END ---"""
        ...
        response = client.models.generate_content(
            model=self.api_cfg.gemini_model_fast,
            contents=[prompt],
        )
