...
def _strip_heavy_fields_for_pickle(doc: Any) -> Any:
    """
    Return a deep-copied doc with heavy-but-regenerable fields removed.

    Only page.rendered_image is stripped -- it's the full-page raster used
    transiently during extraction/page-type classification and is not
    needed by anything downstream (it can also be very large, hence the
    "avoid huge pickles" comment on the field itself in extractor.py).

    ImageBlock.image_bytes is deliberately KEPT. It is the actual per-block
    cropped image payload -- the thing image_enhancer.py, ocr_corrector.py,
    and diagram_analyzer.py all operate on. An earlier version of this
    function zeroed it out too (`ib.image_bytes = b""`), which meant a
    "cached extraction" had none of the image content the rest of the
    pipeline actually needs -- any code path that loaded from this cache
    would either crash decoding empty bytes or silently produce blank
    output. That is almost certainly why this cache was never wired into
    pipeline.py despite being fully built: it didn't actually work.
    """
    try:
